...
export function generateBackupSnapshot() {
  ...
  const members = db.prepare('SELECT id, bial_id, year, sl_no, name, created_at FROM members').all();
...
router.post('/restore', upload.single('backup_file'), (req, res) => {
  ...
  try {
    ...
    if (Array.isArray(members)) {
      const mStmt = db.prepare('INSERT INTO members (id, bial_id, year, sl_no, name, created_at) VALUES (?, ?, ?, ?, ?, ?)');
      members.forEach(m => mStmt.run(m.id, m.bial_id, m.year || 2026, m.sl_no, m.name, m.created_at));
    }
